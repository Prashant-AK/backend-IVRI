...
const DoctorCallStatus = require("../models/DocCallStatus");
...
router.get("/doc-pending-calls/:id", async (req, res) => {
  try {
    const data = await DoctorCallStatus.find({ docId: req.params.id });
    console.log(data);
    res.status(200).send(data);
  } catch (error) {
    console.log(error);
...
router.get("/doc-accept-call/:id", async (req, res) => {
  try {
    await DoctorCallStatus.findByIdAndUpdate(req.params.id, {
      callReq: true,
    });
...
// user send request for call to the doctor

router.post("/setcallstatus", (req, res) => {
  DoctorCallStatus.create({
